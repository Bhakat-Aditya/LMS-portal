...
const NoticeBoard = () => {
  ...
  const handleDeleteNotice = async (noticeId) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this notice permanently?",
      )
    )
      return;
    try {
  ...
  const handleVote = async (noticeId, optionId) => {
    ...
    try {
      const response = await api.post(`/notices/${noticeId}/vote`, {
        optionId,
      });
  ...
  const handleAddComment = async (noticeId) => {
    ...
    try {
      const response = await api.post(`/notices/${noticeId}/comment`, {
        commentText: text,
      });
      setNotices(notices.map((n) => (n._id === noticeId ? response.data : n)));
  ...
  const handleDeleteComment = async (noticeId, commentId) => {
    ...
    try {
      const response = await api.delete(
        `/notices/${noticeId}/comment/${commentId}`,
      );
      setNotices(notices.map((n) => (n._id === noticeId ? response.data : n)));
    } catch (err) {
  ...
  const handleAddReply = async (noticeId, commentId) => {
    ...
    try {
      const response = await api.post(
        `/notices/${noticeId}/comment/${commentId}/reply`,
  ...
  const handleDeleteReply = async (noticeId, commentId, replyId) => {
    ...
    try {
      const response = await api.delete(
        `/notices/${noticeId}/comment/${commentId}/reply/${replyId}`,
      );
      setNotices(notices.map((n) => (n._id === noticeId ? response.data : n)));
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium transition-colors">
            Stay updated with corporate announcements, active community polls,
            and course notifications.
          </p>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 mb-8 shadow-sm transition-all duration-300">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-extrabold text-gray-900 dark:text-gray-100 text-base transition-colors">
          ...
          {formOpen && (
            <form
              ...
              className="mt-5 border-t border-gray-200 dark:border-gray-850 pt-5 space-y-4 animate-fade-in"
            >
                  ...
                  <div className="space-y-2.5">
                    ...
                    {pollOptions.map((opt, index) => (
                      <div key={index} className="flex items-center gap-2">
                        <span className="text-xs font-extrabold text-gray-400 dark:text-gray-600 font-mono w-5">
                          #{index + 1}
                        </span>
                        <input
                          type="text"
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-16 text-center shadow-sm transition-all duration-300">
          ...
          <p className="text-gray-500 dark:text-gray-300 text-sm font-bold">
            The Notice Board is silent
          </p>
          <p className="text-gray-400 dark:text-gray-500 text-xs mt-1">
            Announcements, bulletins, and community polls will appear here.
          </p>
        </div>
      ) : (
              ...
              >
                <div className="p-6 flex flex-col gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <p className="font-extrabold text-gray-900 dark:text-gray-100 text-sm transition-colors">
                            {notice.authorName}
                          </p>
                  ...
                  {isPoll && (
                    <div className="bg-gray-50 dark:bg-gray-950 border border-gray-150 dark:border-gray-800 rounded-xl p-5 mt-1 space-y-4 transition-colors">
                              ...
                              {hasVoted ? (
                                ...
                                <div
                                  className={`relative border rounded-xl p-3 bg-white dark:bg-gray-900 transition-all overflow-hidden flex flex-col gap-1.5 ${
                                    userSelectedThis
                                      ? "border-green-300 dark:border-green-800 ring-2 ring-green-50 dark:ring-green-950/20"
                                ...
                                >
                                  ...
                                  <div className="flex justify-between items-center text-xs font-semibold relative z-10">
                                    <span
                                      className={`truncate flex items-center gap-1.5 ${userSelectedThis ? "text-green-800 dark:text-green-400 font-extrabold" : "text-gray-700 dark:text-gray-300"}`}
                                    >
                                      {opt.optionText}
                                      {userSelectedThis && (
                                        <span className="text-[10px] bg-green-100 dark:bg-green-950/80 text-green-700 dark:text-green-400 font-black px-1.5 py-0.25 rounded">
                                          Your Choice
                                        </span>
                                      )}
                                    </span>
                      ...
                      <div className="flex justify-between items-center text-[10px] text-gray-400 dark:text-gray-500 font-bold font-mono">
                        ...
                        {!user && (
                          <span className="text-red-400 dark:text-red-400/80">
                  ...
                  <div className="flex items-center gap-4 border-t border-gray-100 dark:border-gray-800 pt-4 mt-2">
                    <button
                      onClick={() =>
                        setExpandedNoticeId(isExpanded ? null : notice._id)
                      }
                            ...
                            >
                              ...
                              <div className="flex justify-between items-start gap-4">
                                <div className="min-w-0">
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <span
                                      className={`font-extrabold text-xs ${isCommentTeacher ? "text-indigo-800 dark:text-indigo-400" : "text-gray-800 dark:text-gray-200"}`}
                                    >
                                      {comment.userName}
                                    </span>
                                    {isCommentTeacher && (
                                      ...
                                      return (
                                        ...
                                        >
                                          <div className="min-w-0">
                                            <div className="flex items-center gap-1.5 flex-wrap">
                                              <span
                                                className={`font-bold ${isReplyTeacher ? "text-indigo-800 dark:text-indigo-400 font-extrabold" : "text-gray-800 dark:text-gray-200"}`}
                                              >
                                                {reply.userName}
                                              </span>
                                              {isReplyTeacher && (
                    ...
                    {user ? (
                      <div className="flex gap-2 items-start mt-2 border-t border-gray-200 dark:border-gray-800 pt-4">
                        ...
                        <button
                          ...
                          disabled={
                            isCommenting ||
                            !newCommentText[notice._id] ||
                            newCommentText[notice._id].trim() === ""
                          }
