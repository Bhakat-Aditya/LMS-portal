...
const Toast = ({ msg, type }) => {
  ...
  return (
    <div
      className={`border rounded-md px-4 py-2 text-sm ${c} transition-colors`}
    >
      {msg}
    </div>
  );
...
const Card = ({ title, children }) => (
  <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm p-6 transition-all duration-300">
    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-5 pb-3 border-b dark:border-gray-800 transition-colors">
      {title}
    </h2>
    {children}
  </div>
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 border border-gray-250 dark:border-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] flex flex-col transition-colors duration-300">
        <div className="flex items-center justify-between p-6 border-b dark:border-gray-850">
          <div>
            <h3 className="font-bold text-gray-900 dark:text-gray-100 text-lg transition-colors">
              {course.title}
            </h3>
          ...
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 text-2xl font-light select-none cursor-pointer"
          >
            ×
          </button>
        </div>
          ...
          {loading ? (
            ...
            <ul className="space-y-2">
              {students.map((s, i) => (
                <li
                  key={s._id}
                  className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-950/60 border border-gray-100 dark:border-gray-850 transition-colors"
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-150 truncate transition-colors">
                      {s.name}
                    </p>
...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    if (user?.role === "teacher") fetchMyCourses();
  }, [user]);

  // Fetch chapters when quiz course selection changes
  useEffect(() => {
    if (!quizForm.selectedCourseId) {
      setQuizChapters([]);
      return;
    }
    api
      .get(`/courses/${quizForm.selectedCourseId}/chapters-list`)
  ...
  const totalRevenue = myCourses.reduce(
    (s, c) => s + (c.price ?? 0) * (c.enrollmentCount ?? 0),
    0,
  );
  ...
  const handleCourseChange = (e) =>
    setCourseForm((p) => ({ ...p, [e.target.id]: e.target.value }));
  ...
  const handleCreateCourse = async (e) => {
    ...
    try {
      ...
      setCourseMsg({
        text: "✅ Course created! Publish it to make it visible to students.",
        type: "success",
      });
      setCourseForm({
        title: "",
  ...
  const handleChapterChange = (e) =>
    setChapterForm((p) => ({ ...p, [e.target.id]: e.target.value }));
  ...
  const handleAddChapter = async (e) => {
    ...
    try {
      ...
      setChapterMsg({
        text: err.response?.data?.message || "Failed to add chapter.",
        type: "error",
      });
    } finally {
      setChapterSubmitting(false);
  ...
  const handleQuizChange = (e) =>
    setQuizForm((p) => ({ ...p, [e.target.id]: e.target.value }));
  ...
  const handleAddQuiz = async (e) => {
    ...
    try {
      ...
      setQuizMsg({
        text: err.response?.data?.message || "Failed to add question.",
        type: "error",
      });
    } finally {
      setQuizSubmitting(false);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="mb-6 flex justify-between items-start gap-4">
        <div>
          <h1 className="text-3xl font-black text-gray-900 dark:text-gray-100 transition-colors">
            Teacher Dashboard
          </h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div
            key={s.label}
            className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 text-center shadow-sm min-w-0 transition-all duration-300"
          >
            <p
              className={`text-2xl sm:text-3xl font-black ${s.colour} truncate`}
              title={s.value}
            >
              {s.value}
            </p>
            <p
              className="text-xs text-gray-500 dark:text-gray-450 mt-1 font-semibold truncate"
              title={s.label}
            >
              {s.label}
            </p>
          ...
          <Card title="Add Chapter">
            <form onSubmit={handleAddChapter} className="space-y-4">
              <div>
                ...
                <select
                  id="selectedCourseId"
                  value={chapterForm.selectedCourseId}
              ...
              <Field
                ...
                onChange={handleChapterChange}
                placeholder="Paste direct .mp4 link OR YouTube watch link"
                required
              />
          ...
          <Card title="Add Quiz Question">
            <form onSubmit={handleAddQuiz} className="space-y-4">
              ...
              <div>
                ...
                <select
                  id="selectedCourseId"
                  value={quizForm.selectedCourseId}
                  onChange={(e) =>
                    setQuizForm((p) => ({
                      ...p,
              ...
              <div>
                ...
                <select
                  ...
                  value={quizForm.selectedChapterId}
                  onChange={(e) =>
                    setQuizForm((p) => ({
                      ...p,
                  ...
                  required
                  disabled={!quizForm.selectedCourseId}
            ...
            {coursesLoading ? (
              ...
              <ul className="space-y-4">
                {myCourses.map((course) => (
                  <li
                    key={course._id}
                    className="border border-gray-100 dark:border-gray-800 rounded-xl p-4 bg-white dark:bg-gray-900/50 hover:bg-gray-50 dark:hover:bg-gray-950/40 transition-colors"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-900 dark:text-gray-100 text-sm truncate transition-colors">
                          {course.title}
                        </p>
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {[
                            {
                              icon: "📚",
                              val: `${course.chapterCount ?? 0} ch`,
                              colour:
                                "bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 border border-transparent dark:border-blue-900/30",
                            },
                            {
                              icon: "👥",
                              val: `${course.enrollmentCount ?? 0} students`,
                              colour:
                                "bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-400 border border-transparent dark:border-indigo-900/30",
                            },
                            {
                              icon: "💰",
                              val: `₹${((course.price ?? 0) * (course.enrollmentCount ?? 0)).toLocaleString("en-IN")}`,
                              colour:
                                "bg-orange-50 dark:bg-orange-950/40 text-orange-700 dark:text-orange-400 border border-transparent dark:border-orange-900/30",
                            },
                          ].map((s) => (
                            <span
                              key={s.val}
                              className={`text-xs px-2 py-0.5 rounded-full font-semibold ${s.colour} transition-colors`}
                            >
                              {s.icon} {s.val}
                            </span>
                          ))}
